...
*   **`requirements.txt` (環境依賴清單)**
    *   **功能**：列出本專案運行所需之第三方 Python 函式庫（如 `google-genai`, `notion-client`, `arxiv` 等），以確保開發環境與雲端部署環境之套件一致性。

*   **`.env` (環境變數設定檔)**
    *   **功能**：集中管理機密憑證（Credentials）。包含 API 金鑰與 Notion Database IDs。基於資訊安全規範，此檔案已加入 `.gitignore` 避免外洩。

*   **`.github/workflows/daily_agent.yml` (CI/CD 自動化腳本)**
